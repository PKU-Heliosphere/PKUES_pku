...
if isavePola==1
  matrix=[pas',real(wws(:,1,1)),imag(wws(:,1,1)),real(Pola_norm(:,1,1,1)),imag(Pola_norm(:,1,1,1)),...
      ...
      real(xinorm(:,3,1)),imag(xinorm(:,3,1)),real(xinorm(:,1,1)),imag(xinorm(:,1,1)),...
      real(xinorm(:,2,1)),imag(xinorm(:,2,1)),...
      JE(:,1,3,1),JE(:,2,3,1),JE(:,3,3,1),JE(:,1,1,1),JE(:,2,1,1),JE(:,3,1,1),JE(:,1,2,1),JE(:,2,2,1),JE(:,3,2,1)];
  save(Pola_FileName,'matrix','-ascii');
%   matrix=[pas',real(wws(:,1,1)),imag(wws(:,1,1)),real(Pola_SI(:,1,1,1)),imag(Pola_SI(:,1,1,1)),...
%       real(Pola_SI(:,1,1,2)),imag(Pola_SI(:,1,1,2)),real(Pola_SI(:,1,1,3)),imag(Pola_SI(:,1,1,3)),...
%       real(Pola_SI(:,1,1,4)),imag(Pola_SI(:,1,1,4)),real(Pola_SI(:,1,1,5)),imag(Pola_SI(:,1,1,5)),...
%       real(Pola_SI(:,1,1,6)),imag(Pola_SI(:,1,1,6)),real(dV(:,1,3,1)),imag(dV(:,1,3,1)),...
%       real(dV(:,2,3,1)),imag(dV(:,2,3,1)),real(dV(:,3,3,1)),imag(dV(:,3,3,1)),...
%       real(dV(:,1,1,1)),imag(dV(:,1,1,1)),real(dV(:,2,1,1)),imag(dV(:,2,1,1)),...
%       real(dV(:,3,1,1)),imag(dV(:,3,1,1)),real(dV(:,1,2,1)),imag(dV(:,1,2,1)),...
%       real(dV(:,2,2,1)),imag(dV(:,2,2,1)),real(dV(:,3,2,1)),imag(dV(:,3,2,1))];
%   save(PolaSI_FileName,'matrix','-ascii');
end
...
for jpl=1:npl1
    if(ipa==ipb) % plot 1D polarizations, to update. 2018-10-19 18:10
     ...
     % Mhelicity = 2*cos(theta*pi/180)*1i*tempBx./tempBy./(cos(theta*pi/180)^2+abs(tempBx).^2./abs(tempBy).^2);
      ...
      h2 = figure('unit','normalized','Position',[0.01 0.45 0.5 0.3],...
  'DefaultAxesFontSize',15);
      subplot(121)
       hold on; box on;
      plot(pas,Mcompress,'-',...
...
%       run ../modules/pkues_plot_comp_sigmar_and_sigmac
...
%       run ../modules/pkues_add_polarization_3
