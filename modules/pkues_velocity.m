...
for s=1:S
    ...
    dVnorm(jpa,1,s,jpl)=dV(jpa,1,s,jpl)/vAs(1);
    dVnorm(jpa,2,s,jpl)=dV(jpa,2,s,jpl)/vAs(1);
    dVnorm(jpa,3,s,jpl)=dV(jpa,3,s,jpl)/vAs(1);
    %density fluctuation
    xinorm(jpa,s,jpl) = (dV(jpa,1,s,jpl)*kx+dV(jpa,3,s,jpl)*kz)/wtmp;
    xi_SI(jpa,s,jpl) = xinorm(jpa,s,jpl)*ns0(s); % XYZhu 2021-02-20 , revised 2021-05-04
    Zp_norm(jpa,1,s,jpl) = dVnorm(jpa,1,s,jpl) + Pola_norm(jpa,1,jpl,4) * sqrt(ns0(1)/ns0(s));
    Zp_norm(jpa,2,s,jpl) = dVnorm(jpa,2,s,jpl) + Pola_norm(jpa,1,jpl,5) * sqrt(ns0(1)/ns0(s));
    Zp_norm(jpa,3,s,jpl) = dVnorm(jpa,3,s,jpl) + Pola_norm(jpa,1,jpl,6) * sqrt(ns0(1)/ns0(s));
    Zm_norm(jpa,1,s,jpl) = dVnorm(jpa,1,s,jpl) - Pola_norm(jpa,1,jpl,4) * sqrt(ns0(1)/ns0(s));
    Zm_norm(jpa,2,s,jpl) = dVnorm(jpa,2,s,jpl) - Pola_norm(jpa,1,jpl,5) * sqrt(ns0(1)/ns0(s));
    Zm_norm(jpa,3,s,jpl) = dVnorm(jpa,3,s,jpl) - Pola_norm(jpa,1,jpl,6) * sqrt(ns0(1)/ns0(s));
    % dissipation
