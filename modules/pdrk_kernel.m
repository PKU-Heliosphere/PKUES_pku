  ...
  for jpb = 1:npb % scan 2nd parameter b
    ...
    if(sp==0) % solve all the roots, slow for large N
      ...
      if(iem==1 && iout==2 && icalp==1) % for pdrk_output.m
        ...
        Pola_norm(jpa,jpb,jpl,1:3) = Pola_norm(jpa,jpb,jpl,1:3)/vAs(1);
        ...
        Pola_norm(jpa,jpb,jpl,9) = Pola_SI(jpa,jpb,jpl,9)/1i/(qs(1)*ns0(1)*vAs(1));
        Pola_norm(jpa,jpb,jpl,10) = Pola_SI(jpa,jpb,jpl,10)/1i/(qs(1)*ns0(1)*vAs(1));
        Pola_norm(jpa,jpb,jpl,11) = Pola_SI(jpa,jpb,jpl,11)/1i/(qs(1)*ns0(1)*vAs(1));
