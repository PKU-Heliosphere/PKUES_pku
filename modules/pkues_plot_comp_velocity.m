...
loglog(pas,abs(dVnorm(:,2,1,jpl)./dVnorm(:,1,1,jpl)),'-','Color',pltc(jpl,:),'linewidth',2);
xlim([min(pa),max(pa)]); %ylim([-2.5,1.0]);
xlabel(strpa);ylabel('|\delta V_{y,c}|/|\delta V_{x,c}|');

subplot(362); hold on; box on;
loglog(pas,abs(dVnorm(:,3,1,jpl)./dVnorm(:,1,1,jpl)),'-','Color',pltc(jpl,:),'linewidth',2);
xlim([min(pa),max(pa)]); %ylim([-2.5,1.0]);
xlabel(strpa);ylabel('|\delta V_{z,c}|/|\delta V_{x,c}|');

subplot(363); hold on; box on;
loglog(pas,abs(dVnorm(:,1,1,jpl))./abs(dVnorm(:,1,2,jpl)),'-','Color',pltc(jpl,:),'linewidth',2);
hold on;
plot(pas,zeros(npa)+1,'--','Color','b');
xlim([min(pa),max(pa)]); %ylim([-2.5,1.0]);
xlabel(strpa);ylabel('|\delta V_{x,c}|/|\delta V_{x,b}|');
...
loglog(pas,abs(dVnorm(:,2,2,jpl)./dVnorm(:,1,2,jpl)),'-','Color',pltc(jpl,:),'linewidth',2);
xlim([min(pa),max(pa)]); %ylim([-2.5,1.0]);
xlabel(strpa);ylabel('|\delta V_{y,b}|/|\delta V_{x,b}|');

subplot(368); hold on; box on;
loglog(pas,abs(dVnorm(:,3,2,jpl)./dVnorm(:,1,2,jpl)),'-','Color',pltc(jpl,:),'linewidth',2);
xlim([min(pa),max(pa)]); %ylim([-2.5,1.0]);
xlabel(strpa);ylabel('|\delta V_{z,b}|/|\delta V_{x,b}|');

subplot(369); hold on; box on;
loglog(pas,abs(dVnorm(:,1,2,jpl))./abs(dVnorm(:,1,3,jpl)),'-','Color',pltc(jpl,:),'linewidth',2);
hold on;
plot(pas,zeros(npa)+1,'--','Color','b');
xlim([min(pa),max(pa)]); %ylim([-2.5,1.0]);
xlabel(strpa);ylabel('|\delta V_{x,b}|/|\delta V_{x,e}|');
...
loglog(pas,abs(dVnorm(:,2,3,jpl)./dVnorm(:,1,3,jpl)),'-','Color',pltc(jpl,:),'linewidth',2);
xlim([min(pa),max(pa)]); %ylim([-2.5,1.0]);
xlabel(strpa);ylabel('|\delta V_{y,e}|/|\delta V_{x,e}|');

subplot(3,6,14); hold on; box on;
loglog(pas,abs(dVnorm(:,3,3,jpl)./dVnorm(:,1,3,jpl)),'-','Color',pltc(jpl,:),'linewidth',2);
xlim([min(pa),max(pa)]); %ylim([-2.5,1.0]);
xlabel(strpa);ylabel('|\delta V_{z,e}|/|\delta V_{x,e}|');

subplot(3,6,15); hold on; box on;
loglog(pas,abs(dVnorm(:,1,3,jpl))./abs(dVnorm(:,1,1,jpl)),'-','Color',pltc(jpl,:),'linewidth',2);
hold on;
plot(pas,zeros(npa)+1,'--','Color','b');
xlim([min(pa),max(pa)]); %ylim([-2.5,1.0]);
xlabel(strpa);ylabel('|\delta V_{x,e}|/|\delta V_{x,c}|');
